...
for($i = 2; $i < count($data_array); $i++) {
    $scene_id = $data_array[$i][1];
    if(!$scene_array[$scene_id]){
        $scene_array[$scene_id] = array();
    }
    array_push($scene_array[$scene_id], $data_array[$i][2]);
}
...
$data_num = count($scene_array);
foreach($scene_array as $k => $v) {
    echo "get_scene_transfer_list(";
    print_value($k);
    echo ") ->\n";
    echo "\t[";
    echo implode(", ", $v);
...
echo "get_scene_transfer_list(_) -> [].\n\n";
